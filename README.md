...
2.  **默认信息**：
    - **管理后台端口**：`16601` (仅用于登录管理面板)
    - **管理地址**：`http://你的公网IP:映射后的16601端口`
    - **默认账号**：`666`
    - **默认密码**：`666`

## NAT 小鸡特别说明 (重要)

如果你使用的是 NAT VPS (共享 IP 的小鸡)，请务必阅读以下内容：

- **管理面板**：你需要将内网端口 `16601` 映射到公网的一个可用端口，才能访问管理后台。
- **业务端口**：16601 **仅仅是管理后台**。如果你在 Lucky 中配置了“反向代理”、“WebDAV” 或 “动态域名转发”，你需要在 NAT 面板中**额外映射**这些业务所使用的内网端口到公网。
- **示例**：
    - 想用 Lucky 做 WebDAV，Lucky 内配置内网端口为 `8080`，那么你需要在 NAT 面板添加一条：`内网8080 -> 公网XXXX`。
    - 访问管理面板：`http://公网IP:映射后的16601端口`。
    - 访问 WebDAV：`http://公网IP:映射后的XXXX端口`。
